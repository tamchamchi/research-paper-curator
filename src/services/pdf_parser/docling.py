...
class DoclingParser:
    """Docling PDF for scientific document preprocessing."""
    ...
    def _validate_pdf(self, pdf_path: Path) -> bool:
        ...
        try:
            ...
            if file_size > self.max_file_size_mb:
                logger.warning(
                    f"PDF file size ({file_size / 1024 / 1024:.1f}MB) exceeds limit ({self.max_file_size_mb / 1024 / 1024:.1f}MB), skipping processing"
                )
                raise PDFValidationError(
                    f"PDF file too large: {file_size / 1024 / 1024:.1f}MB > {self.max_file_size_mb / 1024 / 1024:.1f}MB"
                )
    ...
    async def parse_pdf(self, pdf_path: Path) -> Optional[PdfContent]:
        ...
        try:
            ...
            current_section = {
                "title": "Content",
                "content": "",
            }  # Default section if no structure
