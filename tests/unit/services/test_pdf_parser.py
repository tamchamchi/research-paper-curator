...
class TestDoclingParser:
    ...
    def docling_parser(self):
        ...
        return DoclingParser(max_pages=20, max_file_size_mb=10, do_ocr=False, do_table_structure=True)
    ...
    def test_docling_parser_initialization(self, docling_parser):
        ...
        assert docling_parser.max_file_size_mb == 10 * 1024 * 1024
...
class TestPDFParserService:
    ...
    async def test_parse_pdf_success(
        self, mock_parse, pdf_parser_service, valid_pdf_path
    ):
    ...
    @patch("src.services.pdf_parser.parser.DoclingParser.parse_pdf")
    @pytest.mark.asyncio
    async def test_parse_pdf_no_result(
        self, mock_parse, pdf_parser_service, valid_pdf_path
    ):
    ...
    @patch("src.services.pdf_parser.parser.DoclingParser.parse_pdf")
    @pytest.mark.asyncio
    async def test_parse_pdf_docling_error(
        self, mock_parse, pdf_parser_service, valid_pdf_path
    ):
